...
extern RC shutdownBufferPool(BM_BufferPool *const bm)
{
        ...
        Queue *queue = pageFrame[0].QueuePtr;
		...
		int check = pageFrame[i].Pin_Stat; 
                if(check != 0)
                {
                        return RC_ERROR;
                }
		...
		int check = (pageFrame[i].DirtyBit && (pageFrame[i].TotalFix == 0));
		 //if its DirtyBit write it back to the disk.
                if(check == 1)				//If page frame is dirty, then write back to disk
                {
                        ...
                        //ensureCapacity(pageFrame[i].PageNo,&file_handle);
        ...
        switch(s) {
	        	...
	        	case 2:
	                for(int i =0; i<CurQueueSize; i++) {
	                        //if the page already exists in buffer then just increment the fix sum.
	                        if(pageFrames[i].PageNo == pageNum) {
	                                pageFrames[i].TotalFix += 1;
	                                return;
	                        }
	                        //Remove the element with the maximum location value which will be first one which came in to the buffer
	                        if(queue[i].location == CurQueueSize) {
	                                if(pageFrames[i].TotalFix == 0) {

	                                        //change its location to the first and insert the new frame.
	                                        queue[i].location = 1;
	                                        for(int j=0; j<CurQueueSize; j++) {
	                                                if(j != i)
	                                                        queue[j].location+=1;
		...
		int check = (pageFrame[i].PageNo == page->pageNum);
                if(check) {
                        ...
                        if(pageFrame[i].TotalFix> 0)
                                pageFrame[i].TotalFix -= 1;		//decrement the count after removing the page from memory
                        else
                                pageFrame[i].TotalFix=0;
	...
	int i;
		...
		int check = (pageFrame[i].PageNo == page->pageNum);
                if(check)
                {
                        ...
                        SM_FileHandle fh;
                        openPageFile(bm->pageFile, &fh);				//open the page file to write 
                        writeBlock(pageFrame[i].PageNo, &fh, pageFrame[i].Content);	//write the contents to disk
